#!/usr/bin/env python3
"""Monthly housekeeping for the "한국 개최 학회 현황" data.
...
3. --issue-body PATH 를 주면 월간 점검용 GitHub 이슈 본문(마크다운)을 쓴다.
   (특별 관리 13개 학회의 다음 개최지 발표 확인 체크리스트 포함)
...
def main() -> None:
    ...
    if args.issue_body:
        lines = [
            f"매월 자동 생성되는 **한국 개최 학회 데이터 점검** 이슈입니다. ({today.isoformat()} 기준)",
            "",
            "## 현재 등록된 학회",
            "",
            "| 학회 | 개최지 | 일정 | 상태 |",
        ...
        for it in sorted(data["items"], key=lambda i: i["start"]):
            end = datetime.date.fromisoformat(it["end"])
            ...
            lines.append(f"| {it['abbr']} {it['edition']} | {place} "
                         f"| {it['start']} ~ {it['end']} | {status} |")
        if moved:
        ...
        lines += [
            ...
            "",
            "다음 개최지(한국 여부)가 새로 발표되었는지 공식 사이트에서 확인해 주세요.",
            "",
        ]
        tracked = {i["abbr"] for i in data["items"]}
        for abbr in data.get("specialWatch", []):
            mark = " — **현재 한국 개최 등록됨** 🇰🇷" if abbr in tracked else ""
